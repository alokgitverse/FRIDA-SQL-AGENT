...
def create_system_prompt(domain_info: str, schema_string: str, Question_Answer: str):
    ...
    for item in Question_Answer:
        formatted_text += "".join(f"Question: {item.Question}\n SQL Query: {item.SQLQuery}\n Answer: {item.Ans}\n\n")
    formatted_text += "\n"
    try:
        System_Prompt = (
            sql_agent_prompt + "\n\n" +
...
async def upload_file(ques_ans: question_ans):
    schema_string = check_table_schema()
    system_prompt = create_system_prompt(ques_ans.domain_info,schema_string,ques_ans.question_answer)
...
async def run_query(question: str):
    ...
    # erbose=True, use_query_checker=True
    try:
        
        write_query = create_sql_query_chain(llm, db)
        respons_1 = write_query.invoke({"question": question})
